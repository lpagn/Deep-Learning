...
public class Autoencoder {
    ...
    private double[] minNeuronHidden,maxNeuronHidden;
    ...
    public Autoencoder(int nInputs, int nHidden, int nEpochs, double learning_rate, double beta){
        ...
        minNeuronHidden = new double[nHidden];
        maxNeuronHidden = new double[nHidden];
        /*
        //Flasheada
        nHiddenL    = new Neuron[10];
        nHidderR    = new Neuron[10];

        for(int i = 0;i<nHiddenL.length; i++){
            nHiddenL[i] = new Neuron(AutoencoderType.HIDDEN, nInputs, nHidden, beta);
        }
        for(int i = 0;i<nHidderR.length;i++){
            nHidderR[i] = new Neuron(AutoencoderType.INPUT, nInputs, nHidden, beta);
        }*/
    }
    public void train(double[] input){
        ...
        for(int i = 0;i<hiddenPerceptrons.length;i++){
            ...
            if(minNeuronHidden[i] > hiddenPerceptrons[i].getOutput())
                minNeuronHidden[i] = hiddenPerceptrons[i].getOutput();
            if(maxNeuronHidden[i] < hiddenPerceptrons[i].getOutput())
                maxNeuronHidden[i] = hiddenPerceptrons[i].getOutput();
        }
    ...
    private double dotProduct(double[] weights, double[] perceptrons) {
        double sum = 0;

        for(int i = 0 ; i < perceptrons.length ; i++ ) {
            sum += weights[i] * perceptrons[i];
        }
    ...
    public void updateRangeHidden(double[] input){
        normalize(input);
        input = normalize(input);
        boolean hasLearned = true;
        //Print input
